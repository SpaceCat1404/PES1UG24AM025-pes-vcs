...
int index_load(Index *index) {
    ...
    if (!f) return 0; // no index yet — empty staging area is valid
...
int index_save(const Index *index) {
    // Work on a sorted copy so the file is always in deterministic order
    Index sorted = *index;
...
int index_add(Index *index, const char *path) {
    ...
    // Store contents as a blob object
    ObjectID blob_id;
    if (object_write(OBJ_BLOB, buf, (size_t)sz, &blob_id) != 0) {
        free(buf); return -1;
    }
    free(buf);

    // Get file metadata for fast change detection
    struct stat st;
